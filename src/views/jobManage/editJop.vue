<template>
  <div class="jobPublish">
    ...
    <el-button type="primary" @click="publish('publishForm')">修改</el-button>
...
export default {
  ...
  methods: {
    ...
    getDetailInfo(getId){
      this.postAxios("webJob/viewJobDetails",{id:getId}).then(res=>{
        if(res.success){
          for(let key in res.data){
            if(key==="jobProvinces"){
              this.publishForm[key] = res.data[key].split(" ");
            }else{
              this.publishForm[key] = res.data[key];
            }
          }
          let getUE = UE.getEditor(this.getId);
          getUE.execCommand("inserthtml",res.data.jobDescription)
        }
      })
    },
